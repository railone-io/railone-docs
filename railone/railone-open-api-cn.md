...
欢迎使用 Railone API 文档。本文档是针对 Railone ToB 的卡业务，目前支持3个卡种类，分别是F卡、J卡、P卡，对应的虚拟卡是F-V卡、J-V卡、P-V卡，不同卡种支持的法币或手续费不同，使用流程也略有差别。


  | 卡名 |  法币  |           区别            |
  | :--------: | :----: | :------------------------------ |
  |    F、F-V卡（ID：20000001 - 20000008）    |  USD、MYR   |  KYC审核48h以内，KYC填写只支持英文，mobile格式如 +8615821702552，成功会收到邮件，调用激活接口前必须先点击邮件里的链接。到账时间4h内。  |
  |    J、J-V卡（ID：30000001 - 30000008）     | USD、EUR | KYC审核48h以内，KYC填写只支持英文，mobile格式如 +8615821702552，卡片上会印用户的姓名，制卡时间1-2周。激活时必须提交POA或手持卡片自拍照。到账时间4h内。 |
  |   P、P-V卡（ID：40000001 - 40000008）   | EUR       |         KYC时间1h以内，KYC填写只支持英文， mobile格式如 +86-15821702552，country、nationality填两位国家码。到账时间1h内。 |
...
> 建议使用生产环境前先在测试环境调试，生产环境建议先用F卡测试，再根据实际情况测其他类型卡片。
